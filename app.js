const groups = {
  distance: { defaults: ['m', 'ft'], metric: ['m', 'cm', 'km'], american: ['in', 'ft', 'mi'], units: { m: ['Metros', 1], cm: ['Centímetros', .01], km: ['Quilômetros', 1000], in: ['Inches', .0254], ft: ['Pés', .3048], mi: ['Milhas', 1609.344] } },
  weight: { defaults: ['kg', 'lb'], metric: ['g', 'kg'], american: ['oz', 'lb'], units: { g: ['Gramas', .001], kg: ['Quilos', 1], oz: ['Ounces', .028349523125], lb: ['Libras', .45359237] } },
  volume: { defaults: ['l', 'floz'], metric: ['ml', 'l'], american: ['floz', 'cup'], units: { ml: ['Mililitros', .001], l: ['Litros', 1], floz: ['Fluid ounces', .0295735295625], cup: ['Cups', .2365882365] } },
  temperature: { defaults: ['c', 'f'], metric: ['c'], american: ['f'], units: { c: ['Celsius', 1], f: ['Fahrenheit', 1] } }
};
...
function toBase(value, unit) { const g = groups[category]; return category === 'temperature' ? (unit === 'c' ? value : (value - 32) * 5 / 9) : value * g.units[unit][1]; }
function fromBase(value, unit) { const g = groups[category]; return category === 'temperature' ? (unit === 'c' ? value : value * 9 / 5 + 32) : value / g.units[unit][1]; }
